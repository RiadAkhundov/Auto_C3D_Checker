...
% Thanks for the original 2018 version go to: 
% Jarek Tuszynski <jaroslaw.w.tuszynski_at_saic.com>
%
% Updated in 2023 by: Riad Akhundov <Riad.Akhundov@uon.edu.au>
%
%Reads xml files and converts them into Matlab's struct tree.
%
% DESCRIPTION
...
if (~all(~isempty(s)))          % if nothing left than this is probably a number
  ...
  if (option==2)
    ...
    if(isnumeric(num) && numel(num)>0), val=num; end % if a number then save
  end
end
